      ...
      (() => {
        ...
        if (menuButton && mobileNav) {
          menuButton.addEventListener("click", () => {
            ...
            header && header.classList.toggle("has-mobile-open", isOpen);
            syncMenuIcon(isOpen);
          });
          ...
          mobileNav.querySelectorAll("a").forEach((link) =>
            link.addEventListener("click", () => {
              ...
              header && header.classList.remove("has-mobile-open");
              syncMenuIcon(false);
            }),
          );
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        // Observe each counter individually so tall sections still
        // trigger on small mobile screens where 50% of a section
        // never fits in the viewport at once.
        const observer = new IntersectionObserver(
          (entries, observer) => {
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
